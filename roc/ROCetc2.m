function [stSoStat] = ROCetc2(o,labels,bSpeedup)
...
stSoStat.auc = auc(stSoStat.normfp,stSoStat.normtp);
...
%  stSoStat2.tp = stSoStat.fn;
%  stSoStat2.fp = stSoStat.tn;
%  stSoStat2.tn = stSoStat.fp;
%  stSoStat2.fn = stSoStat.tp;
...
stSoStat.auc = auc(stSoStat.normfp,stSoStat.normtp);
% bugfix y08_m08_d22
% if there is no point exactly at .01, then the area calc will be missing
% the right side.
% a better solution is to interpolate a point between the first point less
% than and the first point greater than the threshold.
%%%%
% lfp = find(stSoStat.normfp <= .1);
% stSoStat.auc10 = 10 * auc(stSoStat.normfp(lfp),stSoStat.normtp(lfp));
% lfp = find(stSoStat.normfp <= .01);
% stSoStat.auc100 = 100 * auc(stSoStat.normfp(lfp),stSoStat.normtp(lfp));
% lfp = find(stSoStat.normfp <= .001);
% stSoStat.auc1000 = 1000 * auc(stSoStat.normfp(lfp),stSoStat.normtp(lfp));
% lfp = find(stSoStat.normfp <= .0001);
% stSoStat.auc10000 = 10000 * auc(stSoStat.normfp(lfp),stSoStat.normtp(lfp));
%%%%
stSoStat.auc10 = 10 * auc(stSoStat.normfp,stSoStat.normtp,.1);
stSoStat.auc100 = 100 * auc(stSoStat.normfp,stSoStat.normtp,.01);
stSoStat.auc1000 = 1000 * auc(stSoStat.normfp,stSoStat.normtp,.001);
stSoStat.auc10000 = 10000 * auc(stSoStat.normfp,stSoStat.normtp,.0001);
